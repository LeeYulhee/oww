...
CREATE TABLE `USERS` (
	...
	`user_status`	    VARCHAR(20)	 NOT NULL	 DEFAULT 'NOT_EMAIL'	COMMENT '계정 활성화 상태(ACTIVE, NOT_EMAIL)',
...
CREATE TABLE `NOTIFICATION_SETTINGS` (
	`setting_id`              UUID       NOT NULL    PRIMARY KEY,
	...
	`created_at`              DATETIME   NOT NULL,
	`updated_at`              DATETIME	 NOT NULL    DEFAULT CURRENT_TIMESTAMP,
	`group_id`                UUID       NOT NULL                                 COMMENT '그룹별 알림 설정',
	`user_id`                 UUID       NOT NULL,
	FOREIGN KEY (user_id) REFERENCES USERS(user_id),
...
CREATE TABLE `MEMBER_GOAL_HISTORY` (
	`goal_id`         UUID        NOT NULL    PRIMARY KEY,
